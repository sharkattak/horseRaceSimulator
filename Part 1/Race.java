...
public class Race {
	...
	/**
	 * Start the race The horse are brought to the start and then repeatedly moved
	 * forward until the race is finished
	 */
	...
	public void startRace() {
		...
		Scanner scanner = new Scanner(System.in);
		int totalHorses = 0;
		boolean validTotalHorses = false;

		while (!validTotalHorses) {
			try {
				System.out.println("How many horses do you want in the race? (Enter a number greater than or equal to 1)");
				totalHorses = scanner.nextInt();

				if (totalHorses > 1) {
					validTotalHorses = true;
				} else {
		...
		for (int i = 1; i <= totalHorses; i++) {
			...
			while (!validConfidence) {
				...
				try {
					System.out.println("Enter the confidence level of the horse (between 0.0 and 1.0): ");
					confidence = scanner.nextDouble();
					if (confidence < 0.0 || confidence > 1.0) {
						throw new IllegalArgumentException("Confidence level must be between 0.0 and 1.0");
					}
					validConfidence = true; // Input is valid, exit the loop
		...
		for (int i = 0; i < horsesArray.length; i++) {
			Horse horse = horsesArray[i];
			System.out.println(
					"Horse " + (i + 1) + ": " + horse.getName() + " (Confidence: " + horse.getConfidence() + ")");
		}

		// Start the race
		boolean finished = false;
		Horse winningHorse = null;
		long raceStartTime = System.currentTimeMillis(); // Define raceStartTime here
		...
		if (winningHorse != null) {
		    ...
		    // Calculate the score based on the race completion time
		    int score = calculateScore(raceDuration);
		    // Update leaderboard after each race with the calculated score
		    writeLeaderboard(winner, score);
	...
	// Method to calculate the score based on the race completion time
	private static int calculateScore(long raceDuration) {
	    // Score = 10000 / raceDuration (in milliseconds)
	    return (int) (10000 / raceDuration);
	}
	...
	public static int welcome(Scanner scanner) {
		...
		while (!validOption) {
			...
			try {
				option = Integer.parseInt(input);// input validation to repeat question if user fails to enter a number
													// between 1-3
	...
	public static void displayGameManual() {
		...
		System.out
				.println("   - The probability of a horse moving forward or falling depends on its confidence rating.");
	...
	private static void writeLeaderboardEntry(String entry) {
		try (FileWriter fileWriter = new FileWriter("LEADERBOARD_FILE", true);
